...
function renderConversion() {
  ...
  note.textContent = `1 ${from} ≈ ${fmtRate(convert(1, from, to))} ${to}`;
}
...
function swapCurrencies() {
  ...
  // Recompute rather than just relabeling, so rounding stays honest: the
  // previous result becomes the new amount, converted back the other way.
  const converted = Number.isNaN(amount) ? null : convert(amount, from, to);

  fromSelect.value = to;
  toSelect.value = from;
  if (converted !== null) {
    // Plain fixed-point, no thousands separators — this goes back into an
    // editable input, and the parser only strips a single comma.
    amountInput.value = converted.toFixed(2);
  }

  swapButton.classList.add("is-swapping");
  setTimeout(() => swapButton.classList.remove("is-swapping"), 200);
